...
public:
	...
	void update(int start)
	{
		...
		for (int i = 0; i < graph.size(); i++) // 모든 노드 방문
		{
			int current = find(); // 최소 거리 탐색
			visited[current] = true; // 방문 처리

			for (int j = 0; j < graph.size(); j++) // 거리 갱신
			{
				if (visited[j] || graph[current][j] == INFINITY) // 스킵 조건
				{
					continue;
				}

				int cost = distance[current] + graph[current][j]; // 새로운 거리 계산

				if (cost < distance[j]) // 짧으면 갱신
				{
					distance[j] = cost;
				}
